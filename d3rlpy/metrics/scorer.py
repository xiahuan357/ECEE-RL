...
def continuous_action_diff_scorer_refute_dis(algo: AlgoProtocol, episodes: List[Episode],AddInfor=None) -> float:
    r"""Returns average TD error.
    ...
    total_errors_MSE = []
    total_errors_MAE = []
    # total_p_value_mean_subset=[]
    total_p_value_mean_random=[]
    for episode in episodes:
        for batch in _make_batches(episode, WINDOW_SIZE, algo.n_frames):
            cols = ['dob_mm', 'dob_wk', 'bfacil', 'ubfacil', 'bfacil3', 'mager41', 'mager14', 'mager9', 'restatus',
                    ...
                    'wtgain', 'wtgain_rec', 'cig_1', 'cig_2', 'cig_3', 'rf_ncesar', 'urf_diab', 'urf_chyper',
                    'urf_phyper',
                    'urf_eclam', 'uop_induc', 'uop_tocol', 'uld_meco', 'uld_precip', 'uld_breech', 'md_present',
                    'md_route',
                     'ume_vac', 'rdmeth_rec', 'dmeth_rec', 'attend', 'apgar5', 'apgar5r', 'dplural',
                    'dlmp_mm',
                    'dlmp_yy', 'estgest', 'combgest', 'gestrec10', 'gestrec3', 'dbwt', 'bwtr14', 'bwtr4', 'uca_anen',
                    'uca_spina', 'uca_ompha', 'uca_cleftlp', 'uca_downs', 'f_morigin', 'f_forigin', 'f_meduc',
                    'f_clinest',
            ...
            data = pd.DataFrame(batch.observations,
                                columns=['rewards', 'action', 'dob_mm', 'dob_wk', 'bfacil', 'ubfacil', 'bfacil3',
                                         'mager41', 'mager14', 'mager9', 'restatus', 'mbrace', 'mracerec', 'umhisp',
                                         'mracehisp', 'mar', 'meduc', 'fagecomb', 'ufagecomb', 'fagerec11', 'fbrace',
                                         'fracerec', 'ufhisp', 'fracehisp', 'precare', 'precare_rec', 'uprevis',
                                         'previs_rec', 'wtgain', 'wtgain_rec', 'cig_1', 'cig_2', 'cig_3', 'rf_ncesar',
                                         'urf_diab', 'urf_chyper', 'urf_phyper', 'urf_eclam', 'uop_induc', 'uop_tocol',
                                         'uld_meco', 'uld_precip', 'uld_breech', 'md_present', 'md_route',
                                         'ume_vac', 'rdmeth_rec', 'dmeth_rec', 'attend', 'apgar5', 'apgar5r', 'dplural',
                                         'dlmp_mm', 'dlmp_yy', 'estgest', 'combgest', 'gestrec10', 'gestrec3', 'dbwt',
                                         'bwtr14', 'bwtr4', 'uca_anen', 'uca_spina', 'uca_ompha', 'uca_cleftlp',
                                         'uca_downs', 'f_morigin', 'f_forigin', 'f_meduc', 'f_clinest', 'f_apgar5',
                                         'f_tobaco', 'f_rf_pdiab', 'f_rf_gdiab', 'f_rf_phyper', 'f_rf_ghyper',
                                         'f_rf_eclamp', 'f_rf_ppb', 'f_rf_ppo', 'f_rf_cesar', 'f_rf_ncesar',
                                         'f_ob_cervic', 'f_ob_toco', 'f_ob_succ', 'f_ob_fail', 'f_ol_rupture',
                                         'f_ol_precip', 'f_ol_prolong', 'f_ld_induct', 'f_ld_augment', 'f_ld_steroids',
                                         'f_ld_antibio', 'f_ld_chorio', 'f_ld_mecon', 'f_ld_fintol', 'f_ld_anesth',
                                         'f_md_present', 'f_md_route', 'f_md_trial', 'f_ab_vent', 'f_ab_vent6',
                                         'f_ab_nicu', 'f_ab_surfac', 'f_ab_antibio', 'f_ab_seiz', 'f_ab_inj',
                                         'f_ca_anen', 'f_ca_menin', 'f_ca_heart', 'f_ca_hernia', 'f_ca_ompha',
                                         'f_ca_gastro', 'f_ca_limb', 'f_ca_cleftlp', 'f_ca_cleft', 'f_ca_downs',
                                         'f_ca_chrom', 'f_ca_hypos', 'f_wtgain', 'f_mpcb', 'f_urf_diabetes',
                                         'f_urf_chyper', 'f_urf_phyper', 'f_urf_eclamp', 'f_uob_induct',
                                         'f_uld_meconium', 'f_uld_precip', 'f_uld_breech', 'f_u_forcep', 'f_u_vacuum',
                                         'f_uca_anen', 'f_uca_spina', 'f_uca_omphalo', 'f_uca_cleftlp', 'f_uca_downs',
                                         'matchs', 'recwt', 'cig_rec', 'rf_diab', 'rf_gest', 'rf_phyp', 'rf_ghyp',
                                         'rf_eclam', 'rf_ppterm', 'rf_ppoutc', 'rf_cesar', 'op_cerv',
                                         'op_tocol', 'op_ecvs', 'op_ecvf', 'on_ruptr', 'on_abrup', 'on_prolg',
                                         'ld_induct', 'ld_augment', 'ld_steroids', 'ld_antibio', 'ld_chorio',
                                         'ld_mecon', 'ld_fintol', 'ld_anesth', 'sex', 'ab_vent', 'ab_vent6', 'ab_nicu',
                                         'ab_surfac', 'ab_antibio', 'ca_anen', 'ca_menin', 'ca_heart', 'ca_hernia',
                                         'ca_ompha', 'ca_gastro', 'ca_limb', 'ca_cleftlp', 'ca_cleft', 'ca_hypos'
                                         ])
            # The action and rewards here represent y, z
            model = CausalModel(
                data=data,
                treatment='rewards',
                outcome='action',
            ...
            rl_action = algo.predict(batch.observations[:, 2:177])  # Exclude z , y

            propensity_strat_estimate.value = rl_action

            rewards_uno = 1 - model.refute_estimate(identified_estimand,
                                                    propensity_strat_estimate,
                                                    method_name="random_common_cause", n_jobs=8,
                                                    random_state=123).refutation_result['p_value']


            total_p_value_mean_random.append(rewards_uno)

            data_1 = batch.rewards
            data_0 = batch.actions

            rl_action = algo.predict(batch.observations[:, 2:177])

            diff = ((np.mean(data_1) - np.mean(data_0))- rl_action[0]) ** 2
            diffMAE = ((np.mean(data_1) - np.mean(data_0))- rl_action[0])

            total_errors_MSE.append(diff)
            total_errors_MAE.append(diffMAE)


    logs_rl.info("rl predict ate diff MSE mean:" + str(float(np.mean(total_errors_MSE))))
    logs_rl.info("rl predict ate diff MAE mean:" + str(float(np.mean(total_errors_MAE))))
    logs_rl.info("rl predict ate diff RMSE mean:" + str(float(np.mean(total_errors_MSE)**0.5)))
    # logs_rl.info("rl predict total_p_value_mean_subset:" + str(float(np.mean(total_p_value_mean_subset))))
    logs_rl.info("rl predict total_p_value_mean_random:" + str(float(np.mean(total_p_value_mean_random))))

    return float(np.mean(total_errors_MSE))

def continuous_action_diff_scorer_real_dis(
    algo: AlgoProtocol, episodes: List[Episode],AddInfor=None
) -> float:
    ...
    for episode in episodes:
        ...
        for batch in _make_batches(episode, WINDOW_SIZE, algo.n_frames):
            ...
            data =  pd.DataFrame(batch.observations,
             columns=['rewards','action','dob_mm','dob_wk','bfacil','ubfacil','bfacil3','mager41','mager14','mager9','restatus','mbrace','mracerec','umhisp','mracehisp','mar','meduc','fagecomb','ufagecomb','fagerec11','fbrace','fracerec','ufhisp','fracehisp','precare','precare_rec','uprevis','previs_rec','wtgain','wtgain_rec','cig_1','cig_2','cig_3','rf_ncesar','urf_diab','urf_chyper','urf_phyper','urf_eclam','uop_induc','uop_tocol','uld_meco','uld_precip','uld_breech','md_present','md_route','ume_vac','rdmeth_rec','dmeth_rec','attend','apgar5','apgar5r','dplural','dlmp_mm','dlmp_yy','estgest','combgest','gestrec10','gestrec3','dbwt','bwtr14','bwtr4','uca_anen','uca_spina','uca_ompha','uca_cleftlp','uca_downs','f_morigin','f_forigin','f_meduc','f_clinest','f_apgar5','f_tobaco','f_rf_pdiab','f_rf_gdiab','f_rf_phyper','f_rf_ghyper','f_rf_eclamp','f_rf_ppb','f_rf_ppo','f_rf_cesar','f_rf_ncesar',
            'f_ob_cervic','f_ob_toco','f_ob_succ','f_ob_fail','f_ol_rupture','f_ol_precip','f_ol_prolong','f_ld_induct','f_ld_augment','f_ld_steroids','f_ld_antibio','f_ld_chorio','f_ld_mecon','f_ld_fintol','f_ld_anesth','f_md_present','f_md_route','f_md_trial','f_ab_vent','f_ab_vent6','f_ab_nicu','f_ab_surfac','f_ab_antibio','f_ab_seiz','f_ab_inj','f_ca_anen','f_ca_menin','f_ca_heart','f_ca_hernia','f_ca_ompha','f_ca_gastro','f_ca_limb','f_ca_cleftlp','f_ca_cleft','f_ca_downs','f_ca_chrom','f_ca_hypos','f_wtgain','f_mpcb','f_urf_diabetes','f_urf_chyper','f_urf_phyper','f_urf_eclamp','f_uob_induct','f_uld_meconium','f_uld_precip','f_uld_breech','f_u_forcep','f_u_vacuum','f_uca_anen','f_uca_spina','f_uca_omphalo','f_uca_cleftlp','f_uca_downs','matchs','recwt','cig_rec','rf_diab','rf_gest','rf_phyp','rf_ghyp','rf_eclam','rf_ppterm','rf_ppoutc','rf_cesar','op_cerv',
            'op_tocol','op_ecvs','op_ecvf','on_ruptr','on_abrup','on_prolg','ld_induct','ld_augment','ld_steroids','ld_antibio','ld_chorio','ld_mecon','ld_fintol','ld_anesth','sex','ab_vent','ab_vent6','ab_nicu','ab_surfac','ab_antibio','ca_anen','ca_menin','ca_heart','ca_hernia','ca_ompha','ca_gastro','ca_limb','ca_cleftlp','ca_cleft','ca_hypos'
             ])
            ...
            # logs.info("rewards data_subset_refuter:" + str(rewards))
            logs.info("rewards random_add_unobserved_common_cause:" + str(rewards_uno))



            # total_p_value_mean_subset.append(rewards)
            total_p_value_mean_random.append(rewards_uno)

            data_1 = batch.rewards
            data_0 = batch.actions
            logs.info("real ate:"+str(np.mean(data_1) - np.mean(data_0)))


            logs.info("Base method ate:"+str(propensity_strat_estimate.value))


            diff = ((np.mean(data_1) - np.mean(data_0)) - propensity_strat_estimate.value) ** 2
            diffMAE = ((np.mean(data_1) - np.mean(data_0)) - propensity_strat_estimate.value)



            total_diffs_MSE.append(diff)
            total_diffs_MAE.append(diffMAE)
    # smaller is better
    logs.info("Base method ate diff MSE mean:" + str(float(np.mean(total_diffs_MSE))))
    logs.info("Base method ate diff MAE mean:" + str(float(np.mean(total_diffs_MAE))))
    logs.info("Base method ate diff RMSE mean:" + str(float(np.mean(total_diffs_MSE)**0.5)))
    # logs.info("Base method total_p_value_mean_subset:" + str(float(np.mean(total_p_value_mean_subset))))
    logs.info("Base method total_p_value_mean_random:" + str(float(np.mean(total_p_value_mean_random))))


    return float(np.mean(total_diffs_MSE))
...
def average_value_estimation_scorer(
    ...
    """
    # logs = Logger.Logger('/root/PycharmProjects/paper/logs/' + AddInfor + '_rl_predict.log')
    total_values = []
    for episode in episodes:
        for batch in _make_batches(episode, WINDOW_SIZE, algo.n_frames):
            actions = algo.predict(batch.observations[:,2:177])
            values = algo.predict_value(batch.observations[:,2:177], actions)
            total_values += cast(np.ndarray, values).tolist()
            # logs.info('estimation:'+str(cast(np.ndarray, values)))
    # logs.close()
